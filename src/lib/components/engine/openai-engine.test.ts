...
describe("OpenAiEngine", () => {
  ...
  it("ENG-03: test button appears when openai backend is active", () => {
    bindableConfig.tts.active_backend = "openai";
    render(OpenAiEngine, { localConfig: bindableConfig });

    const testButton = screen.queryByText("Test Engine");
    expect(testButton).toBeTruthy();
  });

  it("ENG-03: test button does not appear when openai backend is not active", () => {
  ...
  it("ENG-03: install card does NOT appear for openai backend", async () => {
    bindableConfig.tts.active_backend = "openai";
    mockInvoke.mockResolvedValue({
      success: false,
      message: "Command not found. Install the TTS engine.",
    ...
    // Install card should NOT appear for OpenAI backend
